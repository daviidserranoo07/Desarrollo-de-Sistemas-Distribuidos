...
void imprimirVectorResultado(double resultado[],int tamanio){
	...
	for(int i=0;i<tamanio;i++){
		printf("\nVectorResultado[%d]: %lf",i+1,resultado[i]);
	}
}
...
calvec_1(char *host,double vector1[],double vector2[], int tamanio,int opcion)
{
	...
	case 5:
		...
		printf("Resultado Cliente: %lf, %lf", vector_arg1.values[0],vector_arg2.values[0]);
...
main (int argc, char *argv[])
{
	...
	host = argv[1];
	double vector1[MAX_SIZE],vector2[MAX_SIZE];
	double* vectorA1, *vectorA2;
	do{
		opcion = menu();
		if(opcion!=9){
			if(opcion<5)tipo=1;
