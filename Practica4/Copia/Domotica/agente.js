...
socket.on('obtenerTemperatura', (data) => {
    ...
    //Comprobamos si se ha modificado el estado del aire
    ...
    }else{//Si no se ha modificado el estado del aire, comprobamos si la temperatura ha bajado de la máxima y en caso de ser asi avisamos
        console.log('Comprobamos...');
        if(data.actual < MAX_TEMPERATURA) socket.emit('temperaturaMaxima',false);
        else socket.emit('temperaturaMaxima',true);
    }
});
...
socket.on('obtenerLuminosidad', (data) => {
    ...
    if(resultado.modificado){
        ...
        if(data.actual >= MAX_LUMINOSIDAD) socket.emit('luminosidadMaxima',true);
        else socket.emit('luminosidadMaxima',false);
    }
});
...
socket.on('obtenerDioxido', (data) => {
    ...
    if(resultado.modificado){
        ...
        if(data.actual < MAX_DIOXIDO) socket.emit('dioxidoMaximo',false);
        else socket.emit('dioxidoMaximo',true);
    }
});
