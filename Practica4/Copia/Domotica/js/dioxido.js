...
document.getElementById('encender-filtro').addEventListener('click', function(event){
    event.preventDefault();
    let fecha = new Date();
    let fechaFormateada = formatearFecha(fecha);
    socket.emit('insertarDioxido', {actual: parseInt(dioxidoActual.textContent), maxima: parseInt(dioxidoMaximo.textContent), minima: parseInt(dioxidoMinimo.textContent),fecha: fechaFormateada, estadoFiltroDioxido: true});
    socket.emit('insertarEventoReciente', {evento: 'El usuario ha encendido el filtro de dioxido de carbono',fecha: fechaFormateada});
    socket.emit('actuadorDioxidoModificado', {actual: parseInt(dioxidoActual.textContent), maxima: parseInt(dioxidoMaximo.textContent), minima: parseInt(dioxidoMinimo.textContent),fecha: fechaFormateada, estadoFiltroDioxido: true});
});

document.getElementById('apagar-filtro').addEventListener('click', function(event){
    event.preventDefault();
    let fecha = new Date();
    let fechaFormateada = formatearFecha(fecha);
    socket.emit('insertarDioxido', {actual: parseInt(dioxidoActual.textContent), maxima: parseInt(dioxidoMaximo.textContent), minima: parseInt(dioxidoMinimo.textContent),fecha: fechaFormateada, estadoFiltroDioxido: false})
    socket.emit('insertarEventoReciente', {evento: 'El usuario ha apagado el filtro de dioxido de carbono',fecha: fechaFormateada});
    socket.emit('actuadorDioxidoModificado', {actual: parseInt(dioxidoActual.textContent), maxima: parseInt(dioxidoMaximo.textContent), minima: parseInt(dioxidoMinimo.textContent),fecha: fechaFormateada, estadoFiltroDioxido: false});
});
