import { formatearFecha, luminosidadActual, luminosidadMaxima, luminosidadMinima, persiana, actualizarValores, modificarEstadoPersiana, subirPersiana, bajarPersiana } from './funciones.js';
...
subirPersiana.addEventListener('click', function(event){
    event.preventDefault();
    let fecha = new Date();
    let fechaFormateada = formatearFecha(fecha);
    socket.emit('insertarLuminosidad', {actual: parseInt(luminosidadActual.textContent), maxima: parseInt(luminosidadMaxima.textContent), minima: parseInt(luminosidadMinima.textContent),fecha: fechaFormateada, estadoPersiana: true});
    socket.emit('insertarEventoReciente', {evento: 'El usuario ha subido la persiana',fecha: fechaFormateada});
    socket.emit('actuadorLuminosidadModificado', {actual: parseInt(luminosidadActual.textContent), maxima: parseInt(luminosidadMaxima.textContent), minima: parseInt(luminosidadMinima.textContent),fecha: fechaFormateada, estadoPersiana: true});
});

bajarPersiana.addEventListener('click', function(event){
    event.preventDefault();
    let fecha = new Date();
    let fechaFormateada = formatearFecha(fecha);
    socket.emit('insertarLuminosidad', {actual: parseInt(luminosidadActual.textContent), maxima: parseInt(luminosidadMaxima.textContent), minima: parseInt(luminosidadMinima.textContent),fecha: fechaFormateada, estadoPersiana: false})
    socket.emit('insertarEventoReciente', {evento: 'El usuario ha bajado la persiana',fecha: fechaFormateada});
    socket.emit('actuadorLuminosidadModificado', {actual: parseInt(luminosidadActual.textContent), maxima: parseInt(luminosidadMaxima.textContent), minima: parseInt(luminosidadMinima.textContent),fecha: fechaFormateada, estadoPersiana:false})
});
