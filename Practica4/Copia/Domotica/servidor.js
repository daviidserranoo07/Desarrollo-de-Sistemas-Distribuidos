    ...
    io.on('connection', (client) => {
        ...
        collectionTemperatura.find().sort({ $natural: -1 }).limit(1).toArray().then((data) => { 
            if (data.length > 0) {
                ...
                client.emit('temperaturaMaximaAviso', data[0].actual >= data[0].maxima);
            }
        }).catch((error) => {
            console.error("Error obteniendo la temperatura: ", error);
        });
        ...
        collectionLuminosidad.find().sort({ $natural: -1 }).limit(1).toArray().then((data) => { 
            if (data.length > 0) {
                ...
                client.emit('luminosidadMaximaAviso', data[0].actual <= data[0].minima);
            }
        }).catch((error) => {
            console.error("Error obteniendo la luminosidad: ", error);
        });
        ...
        collectionDioxido.find().sort({ $natural: -1 }).limit(1).toArray().then((data) => { 
            if (data.length > 0) {
                ...
                client.emit('dioxidoMaximoAviso', data[0].actual >= data[0].maxima);
            }
        }).catch((error) => {
            console.error("Error obteniendo el dioxido: ", error);
        });
        ...
        client.on('temperaturaMaxima', (data) => {
            if(data) io.emit('temperaturaMaximaAviso', data);
            else io.emit('temperaturaMaximaAviso', data);
        });

        client.on('luminosidadMaxima', (data) => {
            if(data) io.emit('luminosidadMaximaAviso', data);
            else io.emit('luminosidadMaximaAviso', data);
        });

        client.on('dioxidoMaximo', (data) => {
            if(data) io.emit('dioxidoMaximoAviso', data);
            else io.emit('dioxidoMaximoAviso', data);
        });
